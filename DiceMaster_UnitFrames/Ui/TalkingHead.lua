...
local talkingHeadFontColor = {
	["Horda"] = {Name = CreateColor(0.28, 0.02, 0.02), Text = CreateColor(0.0, 0.0, 0.0), Shadow = CreateColor(0.0, 0.0, 0.0, 0.0)},
	["Alianza"] = {Name = CreateColor(0.02, 0.17, 0.33), Text = CreateColor(0.0, 0.0, 0.0), Shadow = CreateColor(0.0, 0.0, 0.0, 0.0)},
...
function DiceMasterTalkingHeadFrame_SetUnit(modelID, name, textureKit, message, sound, isWhisper)
	...
	if not Me.db.global.talkingHeads then
		if not isWhisper then
			Me.PrintMessage("|cFFE6E68E"..(name or "Unknown").." dice: "..message, "RAID")
		else
			Me.PrintMessage("|cFFFF7EFF"..(name or "Unknown").." susurra: "..message, "RAID")
		end
		return;
	end
...
function DiceMasterTalkingHeadFrame_PlayCurrent( message, isWhisper )
	...
	if not isWhisper then
		Me.PrintMessage("|cFFE6E68E"..( frame.NameFrame.Name:GetText() or "Unknown").." dice: "..message, "RAID" )
	else
		Me.PrintMessage("|cFFFF7EFF"..( frame.NameFrame.Name:GetText() or "Unknown").." susurra: "..message, "RAID" )
	end
