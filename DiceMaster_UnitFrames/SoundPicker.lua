...
function Me.SoundPicker_OnLoad(self)
	...
	self.Inset:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", -6, 150)
...
function Me.SoundPicker_BindSound( self, sound )
	if not DiceMasterSoundPicker.selected then
		return;
	end
	...
	if list[ DiceMasterSoundPicker.selected ].id then
		...
		local data = { 
			id = list[DiceMasterSoundPicker.selected].id;
			name = list[DiceMasterSoundPicker.selected].name;
		}
		Me.UnitEditing.sounds[ sound ] = data;
	end
end

function Me.SoundPicker_UnbindSound( self, sound )
	self:SetText( "Not Bound" )
	Me.UnitEditing.sounds[ sound ] = nil;
end

function Me.SoundPicker_UnbindAllSounds()
	if Me.UnitEditing then
		Me.UnitEditing.sounds = {}
		DiceMasterSoundPickerPreAggroSound:SetText( "Not Bound" )
		DiceMasterSoundPickerAggroSound:SetText( "Not Bound" )
		DiceMasterSoundPickerMeleeAttackSound:SetText( "Not Bound" )
		DiceMasterSoundPickerRangedAttackSound:SetText( "Not Bound" )
		DiceMasterSoundPickerSpellAttackSound:SetText( "Not Bound" )
		DiceMasterSoundPickerWoundSound:SetText( "Not Bound" )
		DiceMasterSoundPickerDeathSound:SetText( "Not Bound" )
	end
end

function Me.SoundPicker_UpdateBindings()
	if Me.UnitEditing.sounds then
