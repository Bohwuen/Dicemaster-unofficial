...
function Me.RefreshMoraleFrame( reset )
	...
	if Me.IsLeader( false ) then
		DiceMasterMoraleBar:ApplyTextures(Profile.morale.symbol, Profile.morale.name, Profile.morale.tooltip, reset, Profile.morale.color)
		Me.MoraleBar_ShareStatusWithParty()
	end
end
...
function Me.MoraleBar_OnStatusMessage( data, dist, sender )
	...
	DiceMasterMoraleBar:ApplyTextures(data.ms, data.mn, data.mt, data.mv, data.mc)
end
