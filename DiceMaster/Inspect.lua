	...
	if status then
		...
		if store.charges.max > 0 and not ourChargesHack then
			...
			if not symbol:find("charge") then
				...
				DiceMasterInspectFrame.charges2:ApplyTextures( store.charges.symbol, store.charges.name, store.charges.tooltip, store.charges.count, store.charges.color )
		...
		if Me.IsLeader( true ) then
			DiceMaster4.SetupTooltip( DiceMasterInspectFrame.health, nil, "Salud", nil, nil, nil, 
			...
			DiceMaster4.SetupTooltip( DiceMasterInspectFrame.health, nil, "Salud", nil, nil, nil, "Representa la salud del personaje." )
		end
