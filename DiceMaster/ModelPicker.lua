...
StaticPopupDialogs["DICEMASTER4_CREATECOLLECTION"] = {
  text = "Ingresa un nombre para la colección de modelos:",
  button1 = "Aceptar",
  button2 = "Cancelar",
  ...
  OnAccept = function (self, data, data2)
	...
	elseif text~= "" then
		...
		Me.PrintMessage("|TInterface/AddOns/DiceMaster/Texture/logo:12|t \""..text.."\" creado.", "SYSTEM");
		Me.ModelPicker_RefreshGrid()
	else
		UIErrorsFrame:AddMessage( "Nombre inválido.", 1.0, 0.0, 0.0, 53, 5 );
	end
  end,
  hasEditBox = true,
  timeout = 0,
...
StaticPopupDialogs["DICEMASTER4_RENAMECOLLECTION"] = {
  ...
  OnAccept = function (self, data, data2)
	...
	elseif text~= "" then
		...
		Me.PrintMessage("|TInterface/AddOns/DiceMaster/Texture/logo:12|t \""..data.."\" renombrado a \""..text..".\"", "SYSTEM");
...
StaticPopupDialogs["DICEMASTER4_DELETECOLLECTION"] = {
  text = "¿estás seguro de borrar esta colección de modelos?",
  button1 = "Si",
  button2 = "No",
  OnAccept = function (self, data, data2)
	DiceMaster4UF_Saved.MyCollection[data] = nil
	Me.PrintMessage("|TInterface/AddOns/DiceMaster/Texture/logo:12|t \""..data.."\" Borrado.", "SYSTEM");
...
StaticPopupDialogs["DICEMASTER4_ADDTOCOLLECTION"] = {
  text = "ingresa el displayID para el modelo:",
  button1 = "Aceptar",
  button2 = "Cancelar",
  OnAccept = function (self, data)
	...
	if text~= nil and (text <= Me.unitList) then
		...
		Me.PrintMessage("|TInterface/AddOns/DiceMaster/Texture/logo:12|t "..text.." añadido a \""..data..".\"", "SYSTEM");
		Me.ModelPicker_RefreshGrid()
	else
		UIErrorsFrame:AddMessage( "Nombre inválido.", 1.0, 0.0, 0.0, 53, 5 );
	end
	 ...
	 if level == 1 then
	  ...
	  -- Show the "My Collections" sub-menu
		...
		if DiceMaster4UF_Saved.MyCollection then
			for k,v in pairs(DiceMaster4UF_Saved.MyCollection) do
			   if menuList == k then
				   info.text = "|cFF00FF00Añadir modelo..."
				   info.notCheckable = true;
...
function Me.MyCollectionDropDown_OnLoad(frame, level, menuList)
	local info = UIDropDownMenu_CreateInfo()
	info.text = "|cFFffd100Añadir a..."
	info.notClickable = true;
	info.notCheckable = true;
	UIDropDownMenu_AddButton(info, level)
	info.text = "|cFF00FF00Nuevo..."
	info.arg1 = "new";
	info.arg2 = frame;
	info.disabled = false;
