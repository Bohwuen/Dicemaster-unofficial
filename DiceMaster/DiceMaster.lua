...
function Me.RefreshChargesFrame( tooltip, color )
	...
	if tooltip then
		...
		Me.SetupTooltip( DiceMasterChargesFrame.bar, nil, chargesPlural, 
			nil, nil, nil, 
			Profile.charges.tooltip.."|n|cFF707070<Clic izquierdo para añadir "..chargesPlural..">|n"
			.."<Clic derecho para quitar "..chargesPlural..">")
	end
	...
	if not Profile.charges.symbol:find("charge") then
		...
		DiceMasterChargesFrame.bar2:ApplyTextures( Profile.charges.symbol, Profile.charges.name, Profile.charges.tooltip .. "|n|cFF707070<Left Click to Add " .. chargesPlural .. ">|n" .. "<Right Click to Remove " .. chargesPlural .. ">", Profile.charges.count, Profile.charges.color )
...
function Me.RollWheelDropDown_OnClick( self, arg1, arg2, checked )
	...
	if checked then
		...
		if #Me.db.char.rollOptions < 8 then
			...
			UIErrorsFrame:AddMessage( "Solo pueden usarse 8 opciones de tirada al mismo tiempo.", 1.0, 0.0, 0.0, 53, 5 ); 
			CloseDropDownMenus()
		end
...
function Me.RollWheelDropDown_OnLoad( frame, level, menuList )
	...
	if level == 1 then
		info.text = "|cFFffd100Opciones de tirada"
		info.notClickable = true;
		info.notCheckable = true;
		UIDropDownMenu_AddButton(info)
		info.text = "Hab. Mágicas"
		info.disabled = false;
		info.notClickable = false;
		info.hasArrow = true;
		info.menuList = "Hab. Mágicas"
		UIDropDownMenu_AddButton(info)
		info.text = "Hab. Armas"
		info.menuList = "Hab. Armas"
		UIDropDownMenu_AddButton(info)
		info.text = "Hab. Físicas"
		info.menuList = "Hab. Físicas"
		UIDropDownMenu_AddButton(info)
		info.text = "Hab. Mentales"
		info.menuList = "Hab. Mentales"
		UIDropDownMenu_AddButton(info)
		info.text = "Hab. Sociales"
		info.menuList = "Hab. Sociales"
		UIDropDownMenu_AddButton(info)
		info.text = "Defensas"
		info.menuList = "Defensas"
		UIDropDownMenu_AddButton(info)
		info.text = "|cFFFF0000Limpiar todas las opciones"
		info.notClickable = false;
		info.hasArrow = nil;
	...
	elseif menuList then
		for i = 1,#Me.RollList[menuList] do
			...
			if Me.RollList[menuList][i].stat then
				info.tooltipText = Me.RollList[menuList][i].desc .. "|n|cFF707070(Modificado por "..Me.RollList[menuList][i].stat.." + "..info.text..")|r";
			end
			
			for i = 1, #Profile.stats do
				if Profile.stats[i].name == info.text then
					if Profile.stats[i].attribute and Profile.stats[i].desc then
						info.tooltipText = Profile.stats[i].desc .. "|n|cFF707070(Modificado por " .. Profile.stats[i].attribute .. ")|r";
					end
					break
				end
			end
...
function Me.RollWheel_OnEnter( self, rotation )
	...
	if self.Stat then
		GameTooltip:AddLine( "(Modificado por "..self.Stat.." + "..self.Value..")", 0.44, 0.44, 0.44, true )
	end
...
function Me.UnlockFrames()
	...
	if Me.db.global.hideTypeTracker then
		if DiceMasterPostTrackerFrame.Message:GetText() ~= nil then
			DiceMasterPostTrackerFrame.Message:SetText("Nadie está escribiendo.")
		end
